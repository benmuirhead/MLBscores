...
public class GameRange {
	...
	public GameRange(String team, Calendar start, Calendar end) {
		System.out.println("Create GameRange");
		this.start = start;
		this.end = end;
		this.name = team;
	...
	private void findGames() {
		...
		for (int i = 0; i < dateHTML.size(); i++) {// Iterate through the list
													// of addresses
			index = dateHTML.get(i).indexOf(name);// finds the team's name in
													// the HTML file
			if (index != -1) {// the team exists in the HTML file
								// the index will return somewhere in the middle
								// of a string like
								// "gid_2015_04_24_atlmlb_phimlb_1/"
								// trace forwards and backwards until the
								// quotation marks are reached
				start = index;
				end = index;
				for (start = index; dateHTML.get(i).charAt(start) != '"'; start--) {// keep
																					// going
																					// until
																					// the
																					// start
				...
				for (end = index; dateHTML.get(i).charAt(end) != '"'; end++) {// keep
																				// going
																				// until
																				// the
																				// start
				...
				tempAddress = dateAddresses.get(i)
						+ dateHTML.get(i).substring(start + 1, end);
